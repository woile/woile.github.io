...
direction we are going, specifically with **micro-frontends**. 
This pattern, spite of its benefits, I don't think it can be
implemented properly by most teams, and it's not an idea we should keep suggesting.

The Micro-frontends idea, in case you don't know, proposes to have a layer in your architecture, 
that finds the different chunks of code and sends them back to the user. 
Like having a loadbalancer, that fetches different chunks of the frontend, and streams
them to the browser.
...
The main issue to me, is that a frontend application, composed by many chunks belonging to different teams, 
has to be **glued together** at some point, or somewhere. 
Whether you use a micro-frontend architecture or a monorepo, the final
user has to experience one cohesive app, this is **different** than developing api backends, there's no UI there,
...
- Maintain *some* shared state (logged user or is_authenticated or any other herbs), or libraries.
But most of the times teams should be able to add and manage their own global state
- Write tools to assist other teams, like linting rules
...
- Write tests for the layout, and explain how to easily start writing tests

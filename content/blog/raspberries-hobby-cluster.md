...
built with raspberries.

In another post I\'ll describe how to configure a Kubernetes cluster in
our raspberries.
...
- 4x [Raspberry Pi 3
  B+](https://www.amazon.de/gp/product/B07BDR5PDW/ref=oh_aui_detailpage_o04_s01?ie=UTF8&psc=1)
- 4x [RJ45 Cat6 ethernet
  cable](https://www.amazon.de/gp/product/B01AWK81VM/ref=oh_aui_detailpage_o04_s03?ie=UTF8&psc=1)
- 4x [32GB Micro
  SDHC](https://www.amazon.de/gp/product/B06XFSZGCC/ref=oh_aui_detailpage_o04_s02?ie=UTF8&psc=1)
- 4x [Micro USB
  cable](https://www.amazon.de/gp/product/B016BEVNK4/ref=oh_aui_detailpage_o02_s00?ie=UTF8&psc=1)
- 1x [Switch TP-LINK TL-SF1005D 5-Port 10/100Mbps Unmanaged
  Desktop](https://www.amazon.de/gp/product/B000FNFSPY/ref=oh_aui_detailpage_o04_s03?ie=UTF8&psc=1)
- 1x [Anker Port Wall
  Chargers](https://www.amazon.de/gp/product/B00VUGOSWY/ref=oh_aui_detailpage_o04_s04?ie=UTF8&psc=1)
- 1x [USB to
  barrel](https://www.amazon.de/DELOCK-Kabel-USB-Power-Hohlstecker/dp/B001XM49Y2/ref=sr_1_7?s=computers&ie=UTF8&qid=1540141485&sr=1-7&keywords=usb+to+barrel)
  (optional)
...
- flash v2.2.0
  [installation](https://github.com/hypriot/flash/tree/2.2.0#installation)
- hypriot OS v1.9.0
  [download](https://github.com/hypriot/image-builder-rpi/releases/download/v1.9.0/hypriotos-rpi-v1.9.0.img.zip)

## SSH keys

We need a SSH key in order to connect to the cluster without having to
type the password every time we access.
...
You can find a more in depth explanation in this
[tutorial](https://confluence.atlassian.com/bitbucketserver/creating-ssh-keys-776639788.html)

## Creating cloud-init-config

The `flash` command line utility, uses a cloud-init-conf in order to
configure the debian system that it\'s going to be installed. We are
gonna use hypriot debian, which is optimized for containers.

Let\'s generate the conf using a template I\'ve created, remember to
update the environment variables with your values.

| Variable | Description |
| --- | --- |
| `USERNAME` | Username to login into the os |
| `SSH_PUBLIC_KEY` | A public key to log in without typing the password.|
| `WIFI_SSID_NAME` | The name of your wifi |
| `WIFI_PASSWORD` | Use `wpa_passphrase SSID PASSWORD` for preshared key |
...
Flash the `cloud-init-conf.yaml` in all of your SD cards. It\'s
important to change the hostname per raspberry. Pay attention to param
`--hostname`.
...
we previously added the ssh key, we should not need to type a password.
...
Update your user's password with `passwd`. Remember that by default is
`hypriot`.
...
[Ports used by
kubernetes](https://kubernetes.io/docs/setup/independent/install-kubeadm/#check-required-ports)
...
Edit `/etc/default/isc-dhcp-server`, comment out `# INTERFACESv6=""` and
...
This tutorial is based on [Kubernetes: Up and
Running](http://shop.oreilly.com/product/0636920043874.do), [Hypriot
\\\| Setup Kubernetes on a Raspberry Pi Cluster easily the official
way!]() and [Production Hobby
Cluster](https://imti.co/hobby-cluster/#swap).
